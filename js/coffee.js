...
async function fetchCoffee() {
    ...
    try {
        ...
        root.innerHTML = `<div class="error-msg">Не вдалося завантажити каву. <br> <small>Переконайтеся, що бот запущений.</small></div>`;
    }
}
...
function buildCoffeeGrid(coffeeData) {
    ...
    coffeeData.forEach(item => {
        ...
        art.innerHTML = `
            <div class="product-card__image" style="background-image: url('${imgUrl}');"></div>
            <div class="product-card__content">
                <h3 class="product-card__title">${item.name}</h3>
                <p class="product-card__price">${item.price_250} ₴ / 250г</p>
            </div>
        `;
...
function openCoffeePopup(item) {
    ...
    let html = `
        ...
        <div class="popup__details">
            ...
            <p><strong>Смак:</strong> ${item.taste || '-'}</p>
            <p><strong>Обсмаження:</strong> ${item.roast || '-'}</p>
        </div>
        <div class="popup__weights">
            <div class="weight-box">
                <span class="weight-box__label">250г</span>
                <span class="weight-box__price">${item.price_250} ₴</span>
            </div>
            <div class="weight-box">
                <span class="weight-box__label">500г</span>
                <span class="weight-box__price">${item.price_500} ₴</span>
            </div>
            <div class="weight-box">
                <span class="weight-box__label">1кг</span>
                <span class="weight-box__price">${item.price_1000} ₴</span>
            </div>
        </div>
    `;
